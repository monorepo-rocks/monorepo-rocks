...
import { promptAssetsDirectory, promptEntryPoint, promptFeatureSelection, promptWorkerName } from './prompts.js'
...
export async function createWranglerConfig(options: CreateWranglerConfigOptions): Promise<void> {
	...
	if (hasExistingWranglerConfig()) {
		throw cliError('Wrangler configuration already exists. This tool only creates new configuration files.')
	}
	...
	if (fs.existsSync('package.json') && !hasWranglerDependency()) {
		const shouldInstall = await confirm({
			message: 'Wrangler is not installed. Would you like to install it as a dev dependency?',
			default: true,
		})
